...
def main():
    ...
    while True:
        ...
        #print(states[4][7].parent)
        #print(states[6][7].parent)
        #print(states[5][8].parent)
        #getPath(goal)
        #visual.update()
        #visual.update()
        #visual.update()
        ...
        if sig == "2":
            ...
            change = -1
            try:
                obrow = int(input("row of added obstacle"))
                obcol = int(input("col of added obstacle"))
            except:
                obrow = 0
                obcol = 0
            try:
                #updateList = updateObstacle(updateList, obrow, obcol, states)
                updateList = removeObstacle(updateList, obrow, obcol, states)
            except:
                print("invalid location")

            print("update list")
            print(updateList)
            updateCost(states,goal,obrow,obcol,U,start,change)

            ##
            #updateVertex(start,states[14][5],U)
            ##
            for node in updateList:
                #updateCost(states, goal, node.row, node.col, U, start)
                updateVertex(start,node,U)
